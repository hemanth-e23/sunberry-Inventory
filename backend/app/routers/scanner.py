...
def require_forklift_or_admin(current_user: User = Depends(get_current_active_user)) -> User:
    ...
    if current_user.role not in ("forklift", "admin", "supervisor"):
        raise HTTPException(
...
async def create_forklift_request(
    ...
    is_finished = category and category.type == "finished"
    if not is_finished:
        raise HTTPException(status_code=400, detail="Pallet licence scanning is only for finished goods")
    cases_per_pallet = product.default_cases_per_pallet or 40
    production_date = datetime.utcnow()
    expire_years = product.expire_years or 2
    expiration_date = production_date + timedelta(days=365 * expire_years)
    line_id = None
    line_code = None
    line_number = None
    if lot_number:
    ...
    abandoned = db.query(ForkliftRequest).filter(
        ...
        ForkliftRequest.status == "scanning",
    ).all()
    for old_req in abandoned:
        db.query(PalletLicence).filter(
            PalletLicence.forklift_request_id == old_req.id,
            PalletLicence.status.in_(["pending", "missing_sticker"]),
        ).delete(synchronize_session=False)
        old_req.status = "cancelled"
    if abandoned:
        db.flush()

    request_id = f"fr-{int(datetime.utcnow().timestamp() * 1000)}-{uuid.uuid4().hex[:8]}"
    fr = ForkliftRequest(
        id=request_id,
...
async def scan_pallet(
    ...
    if fr.status != "scanning":
        raise HTTPException(status_code=400, detail="Request is no longer in scanning mode")
    if current_user.role == "forklift" and str(fr.scanned_by) != str(current_user.id):
    ...
    all_licences = db.query(PalletLicence).filter(
        PalletLicence.forklift_request_id == request_id,
        PalletLicence.status.in_(["pending"])
...
async def submit_forklift_request(
    ...
    fr.status = "submitted"
    fr.submitted_at = datetime.utcnow()
    db.commit()
    db.refresh(fr)
...
async def update_forklift_request(
    ...
    if not fr:
        raise HTTPException(status_code=404, detail="Forklift request not found")
    if fr.status not in ("scanning", "submitted"):
    ...
    if cases_per_pallet_changed:
        ...
        licences = [pl for pl in fr.pallet_licences if pl.status in ("pending", "missing_sticker")]
        for pl in licences:
...
async def approve_forklift_request(
    ...
    receipt_id = f"rcpt-{int(datetime.utcnow().timestamp() * 1000)}-{uuid.uuid4().hex[:8]}"
...
async def add_pallet_to_request(
    ...
    if existing:
        ...
        if existing.status == "in_stock":
            raise HTTPException(status_code=400, detail="This pallet is already received and in stock")
        if existing.status in ("pending", "missing_sticker"):
