 ...
 *                                     Each element: { staging_item_id, lot_number, quantity_staged,
 *                                                     available, _itemId }
 *   fromCloseOut     {boolean}        true when launched from CloseOutModal
 *   onClose          {function}       called when the modal should close
 *   onSuccess        {function}       called after successful mark-used so parent can refresh
...
const MarkUsedModal = ({
  ...
  // useAppData kept here per project convention (each modal calls it directly)
  useAppData();
  ...
  const handleMarkUsed = async () => {
    ...
    try {
      ...
      const errMsg =
        err.response?.data?.detail || 'Failed to mark as used.';
    ...
    <ModalOverlay onClose={onClose}>
      ...
      >
        <h3 style={{ margin: 0, fontSize: '1.1rem' }}>
          Mark as Used: {item.ingredient_name}
        </h3>
        <p
      ...
      >
        ...
        >
          {submitting ? 'Saving...' : 'Confirm Used'}
        </button>
      </div>
    </ModalOverlay>
  );
