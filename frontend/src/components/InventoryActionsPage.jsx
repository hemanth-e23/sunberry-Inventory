...
const InventoryActionsPage = () => {
  ...
  // Finished goods products that have current inventory (approved receipts with quantity > 0)
  const finishedGoodsProducts = useMemo(() => {
    ...
    return products
      .filter(product => {
        ...
        return category?.type === 'finished' && productIdsWithStock.has(product.id);
      })
      .map(product => ({
        value: product.id,
  ...
  // Update pick qty for one lot (FIFO list); clamp to 0..available
  ...
  const loadShipOutLicences = async () => {
    ...
    try {
      const licences = await fetchPalletLicences({ product_id: productId, status: 'in_stock' });
      const sorted = (licences || []).sort((a, b) => {
        // Sort by expiration date ascending (FIFO), then by sequence
      ...
      if (!sorted.length) {
        ...
        setShipOutLicenceLoadError('');
        // Auto-select pallets based on FIFO pick list quantities
  ...
  const handleCreateLicencePickList = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setShipOutLicences([]);
        setShipOutError('');
        alert('Ship-out pick list created. Forklift can now scan pallets.');
      } else {
        setShipOutError(result.error || 'Failed');
      }
    } catch (e) {
      setShipOutError(e?.message || 'Failed');
    } finally {
  ...
  const renderShipOutTab = () => {
    ...
    // Build ordered list of selected pallets for the forklift pick list display
    ...
    // Move pallet up/down in the order
    ...
    const printPickList = () => {
      const palletRows = selectedPallets.length > 0
        ? selectedPallets.map((pl, i) => `
          <tr>
            <td>${i + 1}</td>
            <td style="font-family:monospace;font-weight:bold">${pl.licence_number || '—'}</td>
            <td>${pl.lot_number || '—'}</td>
            <td>${pl.location || pl.storage_row_id || '—'}</td>
        ...
        <div className="split">
          ...
          <div className="action-list">
            ...
            {shipOutLicences.length > 0 && (
              <>
                {/* Ordered selected pallets */}
                {selectedPallets.length > 0 && (
                  <div style={{ marginBottom: '0.75rem' }}>
                    <div style={{ fontSize: '13px', color: '#6b7280', marginBottom: '6px', fontWeight: 600 }}>
                      Selected for pick ({selectedPallets.length} pallets · {selectedCases} cases):
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', maxHeight: '200px', overflow: 'auto' }}>
                      {selectedPallets.map((pl, i) => (
                        ...
                        >
                          ...
                          <span style={{ color: '#6b7280', fontSize: '12px' }}>· {pl.cases} cs</span>
                ...
                {/* All available pallets (unselected ones shown for adding) */}
